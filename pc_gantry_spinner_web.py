        ...
        with self.lock:
            ...
            try:
                ...
                if sid == 7:
                    ...
                    # DO NOT read back Reg 56 for Servo 8 (Step Mode). It ruins telemetry.
                ...
                with self.lock:
                    if self.ctrl:
                        for sid in [7, 8]:
                            ...
                            try:
                                if sid == 8:
                                    # Skip polling Reg 56 for Servo 8. Mode 3 (Step Mode) does not provide absolute multi-turn position.
                                    continue
                                else:
    ...
    def do_POST(self):
        ...
        if parsed.path == "/api/move":
            ...
            if not ok:
                ...
                logging.info(f"Nudge Physical {direction.upper()}: Servo {sid} -> target_acc {target_acc}")
...
def connect_hardware():
    try:
        ...
        # Skip update_raw_pos for sid 8. Reg 56 in Mode 3 is relative and will overwrite our persisted dead-reckoning state.
        
        hw.start_background_polling()
        logging.info("Connected hardware on /dev/ttyACM0 (Torque OFF on startup)")
    except Exception as e:
...
def main():
    ...
    logging.info("Gantry Controller running on http://0.0.0.0:8085")
    try:
