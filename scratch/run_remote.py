...
TEXT = "Hello Carson! Testing TTS synthesis and raw audio streaming to Pi 4B."

def run():
    print("=== RUNNING TTS STREAM TEST ===")
    wav_bytes = None
    try:
        ...
        with urllib.request.urlopen(req, timeout=15) as resp:
            if resp.status == 200:
                wav_bytes = resp.read()
                print(f"[1] TTS Server (192.168.0.194:8057) returned {len(wav_bytes)} raw WAV bytes!")
    except Exception as e:
        print(f"[1] TTS Server notice: {e}")

    if not wav_bytes:
        print("[1] Generating test WAV audio sample...")
        import struct, math
        sr = 22050
        buf = bytearray()
    ...
    try:
        with urllib.request.urlopen(req, timeout=5) as resp:
            print(f"[2] Pi 4B Response (HTTP {resp.status}): {resp.read().decode()}")
    except Exception as e:
        print(f"[2] Error posting to Pi 4B: {e}")
