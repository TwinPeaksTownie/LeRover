...
def main():
    log = []
    log.append(f"1. Requesting TTS synthesis from workstation ({TTS_SERVER_URL})...")
    ...
    wav_bytes = None
    try:
        with urllib.request.urlopen(tts_req, timeout=15.0) as resp:
            if resp.status == 200:
                wav_bytes = resp.read()
                log.append(f"   Successfully synthesized {len(wav_bytes)} raw WAV bytes from TTS server!")
            else:
                log.append(f"   TTS server returned status {resp.status}")
    except Exception as e:
        log.append(f"   TTS server (192.168.0.194:8057) notice: {e}")

    if not wav_bytes:
        log.append("   Synthesizing dynamic local PCM test chime WAV for verification...")
        import struct, math
        sr = 22050
        buf = bytearray()
    ...
    log.append(f"2. Streaming {len(wav_bytes)} raw WAV bytes to Pi 4B ({PI4B_AUDIO_URL})...")
    ...
    try:
        with urllib.request.urlopen(p_req, timeout=5.0) as p_resp:
            log.append(f"   Pi 4B Response (HTTP {p_resp.status}): {p_resp.read().decode()}")
    except Exception as e:
        log.append(f"   Pi 4B audio streaming error: {e}")

    with open("/tmp/tts_test_result.log", "w") as f:
        f.write("\n".join(log) + "\n")
    print("\n".join(log))
